...
// ---- View: commits-trend (DDB-backed AI vs Human bar chart + merge rate) ----

/**
 * Replaces the CloudWatch native "Commits / Day (AI vs Human)" and "Merge
 * Ratio: AI vs Human" graphs. Reads the attribution store at render time,
 * which avoids the CloudWatch increment-only problem where CI-seeded human
 * items that later upgrade to AI would double-count in CloudWatch metrics.
 *
 * Renders two inline SVGs:
 *   1. Stacked bar chart: AI (accent) + Human (muted) commits per day/week
 *   2. Line chart: AI merge rate and Human merge rate as %
 */
async function renderCommitsTrend(fromIso: string, toIso: string, days: number, p: Palette): Promise<string> {
  let data: any;
  try {
    data = await invokeReceiver('/v1/commits-daily', `from=${fromIso.slice(0, 10)}&to=${toIso.slice(0, 10)}`);
  } catch (err) {
    return emptyState('commit trend', `Attribution store unreachable (${(err as Error).message}). Requires codeburn sync --attribution + CI workflow.`, p);
  }

  const dayBuckets: Array<{ date: string; ai: number; human: number; mergedAi: number; mergedHuman: number }> = data?.days ?? [];
  if (dayBuckets.length === 0) {
    return emptyState('commit trend', 'No COMMIT# items in the selected range. Run `codeburn sync push --attribution` or merge a PR with prism-ai-metrics.yml installed.', p);
  }

  // Weekly bucketing if range > 21 days
  ...
  return `
    <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:8px">
      <div style="flex:1;min-width:130px;border:1px solid ${p.bord};border-radius:6px;padding:8px 12px">
        <div style="color:${p.mut};font-size:10px">AI Commits</div>
        <div style="color:${p.accent};font-size:20px;font-weight:600">${totalAi}</div>
      </div>
      <div style="flex:1;min-width:130px;border:1px solid ${p.bord};border-radius:6px;padding:8px 12px">
        <div style="color:${p.mut};font-size:10px">Human Commits</div>
        <div style="color:${p.fg};font-size:20px;font-weight:600">${totalHuman}</div>
      </div>
      <div style="flex:1;min-width:130px;border:1px solid ${p.bord};border-radius:6px;padding:8px 12px">
        <div style="color:${p.mut};font-size:10px">AI Share</div>
        <div style="color:${p.fg};font-size:20px;font-weight:600">${aiPct}%</div>
      </div>
      <div style="flex:1;min-width:130px;border:1px solid ${p.bord};border-radius:6px;padding:8px 12px">
        <div style="color:${p.mut};font-size:10px">AI Merge Rate</div>
        <div style="color:${p.ok};font-size:20px;font-weight:600">${aiMergeRate}%</div>
      </div>
      <div style="flex:1;min-width:130px;border:1px solid ${p.bord};border-radius:6px;padding:8px 12px">
        <div style="color:${p.mut};font-size:10px">Human Merge Rate</div>
