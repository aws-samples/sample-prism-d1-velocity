...
└── tests/                152 tests; run with pytest
```
...
Better than "write three JSON files", because the interesting content is not the
schema — it is the two things the agent cannot be trusted with.
...
repository before any commit. That value is what makes agent work distinguishable
end to end:
