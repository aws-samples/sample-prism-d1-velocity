...
### 2. GitHub Webhooks — ⚠️ reference implementation, NOT deployed

> **This is sample code, not shipped infrastructure.** The handler lives at `docs/reference/github-webhook-handler/index.ts` and **no CDK stack deploys it**. You would need to host it yourself (Lambda + API Gateway + webhook secret) for any of the events below to exist.
>
> This matters because `prism.d1.commit` has no other producer. Two Lambdas that consumed it — `defect-correlator` and `spec-to-code-calculator` — were deleted as permanent no-ops for exactly this reason. If you deploy this handler, `prism.d1.commit` starts flowing and you may want them back (see git history).

It processes 5 event types with HMAC-SHA256 signature verification.

| GitHub Event | Event Type Emitted | Key Data Extracted |
|-------------|-------------------|-------------------|
| **push** | `prism.d1.commit` | Files changed per commit, AI trailers parsed from each commit message, aggregate AI ratio. ⚠️ Trailer parsing dies with git-hook removal — see [AI Origin Resolution](#ai-origin-resolution-deferred-attribution-join) |
