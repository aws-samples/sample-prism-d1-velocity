...
- **[Team Velocity Dashboard](docs/dashboard-team.html)** ([spec](docs/data-architecture.md#cloudwatch-team-velocity-prism-d1-team-velocity)) — Hybrid layout: DDB-backed custom-widget panels (delivery KPIs, AI-DORA KPIs, eval gates, governance, agents, Continuum security findings) query the events table directly — full 365-day history, honest empty states naming the missing emitter — plus native CloudWatch graphs for attribution-fed series (AI vs human commits, merge ratio, cost by tool/model). Panel access is gated by IAM (`lambda:InvokeFunction` on the velocity widget Lambda). Delivery KPIs are labeled as proxies (merge frequency, PR cycle time, revert rate) until real deploy/incident integrations exist.
