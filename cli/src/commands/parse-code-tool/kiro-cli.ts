...
/** codeburn-compatible token usage shape, shared across parse-code-tool commands. */
export interface CodeburnOutput {
  overview: { cost: number; calls: number; sessions: number };
  models: { name: string; inputTokens: number; outputTokens: number }[];
  projects: { name: string; cost: number; calls: number }[];
}

/**
 * Parse a Kiro CLI session file into codeburn-compatible token usage.
 * Returns `null` when no session id can be resolved; returns a zero-usage
 * object when the session file does not exist.
 */
export async function collectKiroCli(
  sessionIdOpt?: string,
  project?: string,
): Promise<CodeburnOutput | null> {
  const sessionId = sessionIdOpt || process.env.KIRO_SESSION_ID;
  if (!sessionId) return null;

  const filePath = join(getCliSessionsDir(), `${sessionId}.jsonl`);
  let raw: string;
  try { raw = await readFile(filePath, 'utf-8'); } catch {
    // No session file — zero usage
    return { overview: { cost: 0, calls: 0, sessions: 0 }, models: [], projects: [] };
  }
  ...
  const cost = Math.round(calculateCost(model, inputTokens, outputTokens) * 100) / 100;
  const projectName = project || 'unknown';

  return {
    overview: { cost, calls, sessions: 1 },
    models: [{ name: model, inputTokens, outputTokens }],
    projects: [{ name: projectName, cost, calls }],
  };
}
...
export default {
  ...
  async action(opts: { sessionId?: string; project?: string; format: string }) {
    const output = await collectKiroCli(opts.sessionId, opts.project);
    if (!output) {
    ...
    if (opts.format === 'summary') {
      const m = output.models[0];
      console.log('Kiro CLI Token Usage');
      console.log(`  Calls: ${output.overview.calls}`);
      console.log(`  Cost:  $${output.overview.cost}`);
      if (m) console.log(`  ${m.name}: ${m.inputTokens} in / ${m.outputTokens} out`);
    } else {
